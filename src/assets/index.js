const WELCOME_IMG = {
  uri:
    'https://images.pexels.com/photos/6101716/pexels-photo-6101716.jpeg?auto=compress&cs=tinysrgb&dpr=2&h=650&w=940',
  cache: 'only-if-cached',
};
const DISH_IMG = {
  uri:
    'https://images.immediate.co.uk/production/volatile/sites/2/2018/11/OnePotPasta-47b5b0a.jpg',
};
const LOGO = require('./logo.jpeg');
const HEART_IMG = require('./heart.png');
const SETTINGS_IMG = require('./settings.png');
const RIGHT_ARROW = require('./right-arrow.png');
const MASTERCARD = require('./master-card.png');
const VISA = require('./visa.png');
const APPLE_PAY = require('./apple-pay.png');
const CASH_ON_DELIVERY = require('./cod.png');
export {WELCOME_IMG, LOGO, DISH_IMG, HEART_IMG, SETTINGS_IMG, RIGHT_ARROW, APPLE_PAY, MASTERCARD, VISA, CASH_ON_DELIVERY};
